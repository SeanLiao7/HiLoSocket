        ...
        private static void CheckIfCanBeDeserialized( byte[ ] bytes )
        {
            if ( bytes == null )
                throw new ArgumentNullException( nameof( bytes ),
                    $"輸入參數沒東西可以反序列化喔，類別名稱 : {nameof( JsonCommandFormatter<TCommandModel> )}。" );

            if ( bytes.Length == 0 )
                throw new ArgumentException(
                    $"資料長度不能為零啦，類別名稱 : {nameof( JsonCommandFormatter<TCommandModel> )}。",
                    nameof( bytes ) );
        }
        ...
        private static void CheckIfCanBeSerialized( TCommandModel commandModel )
        {
            if ( commandModel == null )
                throw new ArgumentNullException( nameof( commandModel ),
                    $"輸入參數沒東西可以序列化喔，類別名稱 : {nameof( JsonCommandFormatter<TCommandModel> )}。" );
        }
    }
}